...
const getAllBookHandler = (request, h) => {
    ...
    if (name !== undefined) {
        return {
            status: 'success',
            data: {
                books: books.filter((book) => book.name.toLowerCase().includes(name.toLowerCase())).map((n) => ({
                    id: n.id,
                    name: n.name,
    ...
    if (finished == 0) {
        ...
        return {
            status: 'success',
            data: {
                books: books.filter((book) => book.finished === true && book.finished === false).map((book) => ({
                    id: book.id,
                    name: book.name,
                    publisher: book.publisher,
                })),
    ...
    if (reading == 0) {
        ...
        return {
            status: 'success',
            data: {
                books: books.filter((book) => book.reading === true && book.reading === false).map((book) => ({
                    id: book.id,
                    name: book.name,
                    publisher: book.publisher,
                })),
